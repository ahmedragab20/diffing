...
(function () {
  ...
  var SOUND_PRESETS = ['click','toggle','navigate','open','close','success','resolve','send','error','warning','remove'];
  ...
  function playSound(preset) {
    ...
    work.then(function () { try { synth(ctx, preset); } catch (e) {} }).catch(function () {});
  }
  ...
  var HAPTIC_PRESETS = ['success','warning','error','light','medium','heavy','soft','rigid','selection','nudge'];
  ...
  function feedback(haptic, sound) { fireHaptic(haptic || 'selection'); playSound(sound || 'click'); }
  ...
  document.addEventListener('click', function (e) {
    ...
    var el = target && target.closest
      ? target.closest('button, a[href], [role="button"], input[type="checkbox"], [role="option"]')
      : null;
    ...
    if (state.audio) {
      ...
      if (ctx) {
        var work = ctx.state === 'suspended' ? ctx.resume() : Promise.resolve();
        work.then(function () { try { synth(ctx, isCheckbox ? 'toggle' : 'click'); } catch (er) {} }).catch(function () {});
      }
    }
  }, true);
  ...
  audioToggle.addEventListener('click', function () {
    state.audio = !state.audio; lsSet(LS.audio, state.audio ? 'on' : 'off'); syncToggle(audioToggle, state.audio);
    if (state.audio) playSound('toggle');
  });
  hapticsToggle.addEventListener('click', function () {
    state.haptics = !state.haptics; lsSet(LS.haptics, state.haptics ? 'on' : 'off'); syncToggle(hapticsToggle, state.haptics);
    fireHaptic('medium');
  });
  ...
  // ════════════════════════════════════════════════════════════════════
  var THEMES = [
    { id: 'nord',             name: 'Nord',              bg: '#2e3440', sec: '#242933', acc: '#88c0d0', ext: '#b48ead' },
    { id: 'catppuccin-mocha', name: 'Catppuccin Mocha',  bg: '#1e1e2e', sec: '#181825', acc: '#cba6f7', ext: '#f5c2e7' },
    { id: 'tokyo-night',      name: 'Tokyo Night',       bg: '#1a1b26', sec: '#16161e', acc: '#7aa2f7', ext: '#bb9af7' },
    { id: 'dracula',          name: 'Dracula',           bg: '#282a36', sec: '#1e1f29', acc: '#bd93f9', ext: '#ff79c6' },
    { id: 'rose-pine',        name: 'Rosé Pine',         bg: '#191724', sec: '#1f1d2e', acc: '#c4a7e7', ext: '#ea9a97' },
  ];
  var currentTheme = lsGet(LS.theme, 'nord');
  if (!THEMES.some(function (t) { return t.id === currentTheme; })) currentTheme = 'nord';
    ...
    THEMES.forEach(function (t) {
      ...
      btn.innerHTML =
        '<div class="swatch-preview">' +
          ...
          '<span style="background:' + t.sec + '"></span>' +
          '<span style="background:' + t.acc + '"></span>' +
          '<span style="background:' + t.ext + '"></span>' +
        '</div>' +
        '<div class="swatch-label"><span>' + t.name + '</span>' +
          (selected ? '<span class="swatch-check" aria-hidden="true">●</span>' : '') +
        '</div>';
      btn.addEventListener('click', function () { applyTheme(t.id, true); });
  ...
  var bootLines = [
    ...
    { html: '▸ opening browser review UI … <span class="ok">ok</span>', cls: '' },
    { html: '<span class="quote">"' + esc(quote.text) + '" — ' + esc(quote.author) + '</span>', cls: '' },
    { html: '<span class="ready">ready ❯</span>', cls: '' },
  ];

  function renderBootInstant() {
    bootScreen.innerHTML = bootLines.map(function (l) { return l.html; }).join('\n') +
  ...
  function runBootTypewriter() {
    ...
    var plains = bootLines.map(function (l) { tmp.innerHTML = l.html; return tmp.textContent; });
    var done = [];
    var li = 0;
    var ci = 0;
  ...
  SOUND_PRESETS.forEach(function (p) {
    ...
    btn.innerHTML = '<span class="sb-name">' + p + '</span><span class="sb-caption">' + SOUND_CAPTIONS[p] + '</span>';
    btn.addEventListener('click', function () {
      // global listener already fires 'click'+selection; add the actual preset + matching haptic
      playSound(p);
      var hapticMap = { success: 'success', error: 'error', warning: 'warning', resolve: 'success', remove: 'heavy', send: 'medium', open: 'light', close: 'soft' };
  ...
  HAPTIC_PRESETS.forEach(function (p) {
    ...
    chip.textContent = p;
    chip.addEventListener('click', function () { fireHaptic(p); });
  ...
  document.querySelectorAll('.copy-btn').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      playSound('success'); fireHaptic('success');
      btn.classList.add('copied');
      var hint = btn.querySelector('.copy-hint');
      var orig = hint ? hint.textContent : null;
      if (hint) hint.textContent = '✓ copied';
      setTimeout(function () { btn.classList.remove('copied'); if (hint) hint.textContent = orig; }, 1400);
    });
  });
  ...
  document.querySelectorAll('[data-scroll-to]').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      if (el) { el.scrollIntoView({ behavior: prefersReduced ? 'auto' : 'smooth', block: 'start' }); playSound('navigate'); }
    });
  });
  ...
  var CMDS = [
    ...
    { c: true,  text: '# Review uncommitted changes (opens the web UI on a TTY)' },
    ...
    { c: true,  text: '# Pick a fixed port (otherwise a random free port is chosen)' },
  ...
  function renderCmds(containerId, list) {
    ...
    list.forEach(function (cmd) {
      ...
      if (cmd.c) {
        ...
        textHtml = '<span class="cmd-text"><span class="cmd-bin">' + esc(first) + '</span>' + esc(cmd.text.slice(first.length)) + '</span>';
      }
      row.innerHTML = textHtml + (cmd.c ? '' : '<span class="cmd-copy">⧉ copy</span>');
      if (!cmd.c) {
        row.setAttribute('role', 'button');
        row.setAttribute('tabindex', '0');
        var doCopy = function () {
          copyText(cmd.text);
          playSound('success'); fireHaptic('light');
          row.classList.add('copied');
          var cp = row.querySelector('.cmd-copy');
          if (cp) cp.textContent = '✓ copied';
          setTimeout(function () { row.classList.remove('copied'); if (cp) cp.textContent = '⧉ copy'; }, 1300);
        };
        row.addEventListener('click', doCopy);
        row.addEventListener('keydown', function (e) { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); doCopy(); } });
      }
  ...
  // ════════════════════════════════════════════════════════════════════
  var SLIDES = [
    { tag: '① Drop-in for git diff', title: 'Same revisions, options, pathspecs', body: '60+ git-compatible flags across 12 categories. Swap git diff → diffing and review in the browser — no new workflow to learn.' },
    { tag: '② Two output modes', title: 'Auto-detected by your terminal', body: 'TTY launches the local web server; a pipe or redirect prints a standard unified patch to stdout. Force either with --web or --terminal.' },
    { tag: '③ Inline review', title: 'Comment right on the diff', body: 'Inline comments anchored to +/- lines or whole files, threaded replies, and "Apply suggestion" from a ```suggestion``` block that auto-resolves the comment.' },
    { tag: '④ Hand off to your agent', title: 'Send to agent — locally', body: 'Send a verdict + note; your agent picks up comments, replies inline, applies fixes, and resolves threads over a local HTTP/SSE + MCP server. Nothing leaves 127.0.0.1.' },
    { tag: '⑤ Plan-first reviews', title: 'Sign-off before code', body: 'Submit a markdown plan and block until a human approves, rejects, or requests changes. Verdicts: pending · approved · changes-requested · rejected.' },
    { tag: '⑥ Local-first & secure', title: 'No account, no cloud', body: 'Binds 127.0.0.1 by default, path-traversal protected (403 on escape), no telemetry. Data under ~/.diffing/<repo>-<hash>/; inactive projects auto-pruned after 14 days.' },
  ];
  ...
  SLIDES.forEach(function (s, i) {
    ...
    slide.innerHTML = '<span class="slide-tag">' + esc(s.tag) + '</span><h3>' + esc(s.title) + '</h3><p>' + esc(s.body) + '</p>';
    ...
    dot.addEventListener('click', function () { goSlide(i); feedback('selection', 'navigate'); });
  ...
  function goSlide(i) {
    ...
    slides.forEach(function (sl, n) { sl.classList.toggle('active', n === carIdx); });
    dots.forEach(function (d, n) { d.classList.toggle('active', n === carIdx); d.setAttribute('aria-selected', n === carIdx ? 'true' : 'false'); });
  }
  function startCarTimer() {
    if (prefersReduced) return;
    stopCarTimer();
    carTimer = setInterval(function () { if (!carPaused) goSlide(carIdx + 1); }, 7000);
  }
  function stopCarTimer() { if (carTimer) { clearInterval(carTimer); carTimer = null; } }

  document.getElementById('car-prev').addEventListener('click', function () { goSlide(carIdx - 1); feedback('selection', 'navigate'); });
  document.getElementById('car-next').addEventListener('click', function () { goSlide(carIdx + 1); feedback('selection', 'navigate'); });
  var carousel = document.getElementById('carousel');
  carousel.addEventListener('mouseenter', function () { carPaused = true; });
  carousel.addEventListener('mouseleave', function () { carPaused = false; });
  carousel.addEventListener('focusin', function () { carPaused = true; });
  carousel.addEventListener('focusout', function () { carPaused = false; });
  carousel.addEventListener('keydown', function (e) {
    if (e.key === 'ArrowLeft') { goSlide(carIdx - 1); feedback('selection', 'navigate'); e.preventDefault(); }
    else if (e.key === 'ArrowRight') { goSlide(carIdx + 1); feedback('selection', 'navigate'); e.preventDefault(); }
  });
  startCarTimer();
  ...
  var COMMANDS = {
    'diffing': [
      { text: 'Auto-detects mode. On a TTY this starts the local web server (binds 127.0.0.1 on a random free', cls: 'out' },
      { text: 'port) and opens your browser to review uncommitted changes. Piped/redirected, it prints a', cls: 'out' },
      { text: 'unified patch to stdout.', cls: 'out' },
    ],
    'diffing --help': HELP_BANNER.map(function (l) { return { text: l, cls: l.indexOf('diffing v0.2.1') === 0 ? 'accent' : 'out' }; }),
    'diffing -h': null, // alias, set below
    'diffing --version': [{ text: 'diffing v0.2.1', cls: 'accent' }],
    'diffing -v': null,
    'diffing --staged': [{ text: 'Reviews staged changes (git diff --staged).', cls: 'out' }],
    'diffing HEAD~3': [{ text: 'Reviews the last 3 commits.', cls: 'out' }],
    'diffing main..feature': [{ text: 'Compares the main and feature branches.', cls: 'out' }],
    'diffing --host 0.0.0.0': [{ text: 'Binds to 0.0.0.0 so other machines on your LAN can open the review.', cls: 'out' }],
    'diffing --terminal': [{ text: 'Forces terminal (stdout patch) mode.', cls: 'out' }],
    'diffing --web': [{ text: 'Forces web (browser UI) mode.', cls: 'out' }],
    'diffing await-review': [
      { text: 'Long-polls the running server until you send your comments from the browser (default', cls: 'out' },
      { text: 'timeout 570s). Exit: 0 received · 2 timeout · 3 no server.', cls: 'out' },
    ],
    'diffing reply': [{ text: 'Usage: diffing reply <commentId> --body <text> [--model <name>]  — posts an agent reply to a comment.', cls: 'out' }],
    'diffing resolve': [{ text: 'Usage: diffing resolve <commentId>  — marks a comment resolved.', cls: 'out' }],
    'diffing comments': [{ text: 'Prints all comments as XML (or JSON with --json; --open filters to open comments).', cls: 'out' }],
    'diffing url': [{ text: 'http://127.0.0.1:<port>  (the running server\'s base URL)', cls: 'accent' }],
    'diffing mcp': [
      { text: 'Starts the diffing MCP server (stdio). Register:', cls: 'out' },
      { text: '{"mcpServers":{"diffing":{"command":"diffing","args":["mcp"]}}}', cls: 'accent' },
      { text: 'Exposes 10 tools: await_review, list_comments, reply_to_comment, resolve_comment, submit_plan,', cls: 'out' },
      { text: 'await_plan_review, list_plans, get_plan, reply_to_plan_comment, resolve_plan_comment.', cls: 'out' },
    ],
    'diffing plan': [
      { text: 'Plan-review subcommands: submit <file> [--title T] [--wait] · await · list · show [<id>] ·', cls: 'out' },
      { text: 'reply <id> --body <text> · resolve <id>. Verdicts: pending, approved, changes-requested, rejected.', cls: 'out' },
    ],
    'diffing plan submit': [{ text: 'Usage: diffing plan submit <file> [--title T] [--source S] [--model M] [--id <id>] [--wait] [--timeout N]  — submit a markdown plan for human review (default --timeout 570).', cls: 'out' }],
    'diffing plan await': [{ text: 'Blocks until the human decides (default timeout 570s). Exit: 0 decision · 2 timeout.', cls: 'out' }],
    'diffing update': [
      { text: 'Checking for updates…', cls: 'out' },
      { text: 'Updating diffing via npm (or pnpm if present): npm install -g diffing@latest', cls: 'ok' },
    ],
    'help': [
      { text: 'Demoable commands:', cls: 'accent' },
      { text: '  diffing                 diffing --help            diffing --version', cls: 'out' },
      { text: '  diffing --staged        diffing HEAD~3            diffing main..feature', cls: 'out' },
      { text: '  diffing --host 0.0.0.0  diffing --web/--terminal  diffing await-review', cls: 'out' },
      { text: '  diffing reply           diffing resolve           diffing comments', cls: 'out' },
      { text: '  diffing url             diffing mcp               diffing update', cls: 'out' },
      { text: '  diffing plan [submit|await]                        clear', cls: 'out' },
    ],
  };
  ...
  var COMPLETIONS = [
    'diffing', 'diffing --help', 'diffing --version', 'diffing --staged',
    'diffing HEAD~3', 'diffing main..feature', 'diffing -- src/',
    'diffing --host 0.0.0.0', 'diffing --port 4317', 'diffing --no-open',
    'diffing --web', 'diffing --terminal',
    'diffing await-review', 'diffing reply', 'diffing resolve', 'diffing comments',
    'diffing url', 'diffing mcp', 'diffing update',
    'diffing plan', 'diffing plan submit', 'diffing plan await', 'diffing plan list', 'diffing plan show',
    'help', 'clear',
  ];
  ...
  function shellPrintPrompt(cmd) {
    ...
    div.innerHTML = '<span class="shell-prompt">~/repo on <span class="branch"> main</span> ❯</span> ' + esc(cmd);
    shellOut.appendChild(div);
  }

  function runShell(raw) {
    var cmd = raw.trim();
    if (cmd === '') { shellPrintPrompt(''); scrollShell(); return; }
    shellPrintPrompt(cmd);
    history.push(cmd); histPos = history.length;

    if (cmd === 'clear') { shellOut.innerHTML = ''; return; }

    // Normalize a couple of common path variants
    var key = cmd;
    if (cmd === 'diffing -- src/' || cmd === 'diffing -- src') {
      shellPrint([{ text: 'Limits the review to the src/ pathspec (passed through to git diff).', cls: 'out' }]); scrollShell(); return;
    }
    if (cmd === 'diffing --port 4317' || /^diffing --port \d+$/.test(cmd)) {
      shellPrint([{ text: 'Binds the web server to a fixed port (otherwise a random free port is chosen).', cls: 'out' }]); scrollShell(); return;
    }
    if (cmd === 'diffing --no-open') {
      shellPrint([{ text: 'Starts the server without auto-opening the browser (prints the URL to copy).', cls: 'out' }]); scrollShell(); return;
    }
    if (cmd === 'diffing plan list') {
      shellPrint([{ text: 'Lists submitted plans with their verdicts (pending/approved/changes-requested/rejected).', cls: 'out' }]); scrollShell(); return;
    }
    if (/^diffing plan show/.test(cmd)) {
      shellPrint([{ text: 'Shows a plan (latest if no id), with inline comments and current verdict.', cls: 'out' }]); scrollShell(); return;
    }

    var resp = COMMANDS[key];
    if (resp) { shellPrint(resp); }
    else {
      shellPrint([{ text: "diffing: '" + cmd + "' is not a diffing command. Try 'diffing --help'.", cls: 'err' }]);
    }
    scrollShell();
  }
  function scrollShell() { shell.scrollTop = shell.scrollHeight; }
  ...
  shellInput.addEventListener('keydown', function (e) {
    if (e.key === 'Enter') {
      ...
      var matches = COMPLETIONS.filter(function (c) { return c.indexOf(v) === 0 && c !== v; });
      if (matches.length === 1) { shellInput.value = matches[0]; playSound('navigate'); }
      else if (matches.length > 1) {
        shellPrintPrompt(v);
        shellPrint([{ text: matches.join('    '), cls: 'accent' }]);
        // longest common prefix
        var lcp = matches.reduce(function (a, b) { var i = 0; while (i < a.length && i < b.length && a[i] === b[i]) i++; return a.slice(0, i); });
        shellInput.value = lcp;
        scrollShell();
      }
    }
  });
  shell.addEventListener('click', function (e) { if (e.target === shell || e.target === shellOut) shellInput.focus(); });
  ...
  var DIFF_FILES = [
    {
      ...
      rows: [
        { type: 'ctx', ln: '41', code: '  const args = parseArgs(process.argv.slice(2))' },
        { type: 'ctx', ln: '42', code: '  const isTTY = process.stdout.isTTY' },
        { type: 'del', ln: '43', code: '  const port = 4317' },
        { type: 'add', ln: '43', code: '  // bind 127.0.0.1 on a random free port unless --port is given', suggest: false },
        { type: 'add', ln: '44', code: '  const port = args.port ?? await findFreePort()' },
        { type: 'ctx', ln: '45', code: '  if (isTTY && !args.terminal) {' },
        { type: 'add', ln: '46', code: '    await playStartupDisplay()', suggest: true, suggestion: '    await playStartupDisplay() // 1 of 31 quotes' },
        { type: 'ctx', ln: '47', code: '    return startServer({ host: args.host ?? "127.0.0.1", port })' },
        { type: 'ctx', ln: '48', code: '  }' },
      ],
    },
    {
      ...
      rows: [
        ...
        { type: 'del', ln: '96', code: "      note(700, 'sine', 0.16, 0, 0.03)" },
        { type: 'add', ln: '96', code: "      note(700, 'sine', 0.16, 0, 0.03, 350) // glide down to 350Hz" },
        { type: 'ctx', ln: '97', code: '      break' },
        { type: 'ctx', ln: '98', code: "    case 'success':" },
        { type: 'add', ln: '99', code: "      note(523, 'sine', 0.17, 0, 0.10)    // C5", suggest: false },
        { type: 'add', ln: '100', code: "      note(784, 'sine', 0.17, 0.09, 0.13) // G5" },
      ],
    },
  ];
  ...
  function renderDiff() {
    ...
    DIFF_FILES.forEach(function (f, fi) {
      ...
      var rowsHtml = f.rows.map(function (r, ri) {
        var sign = r.type === 'add' ? '+' : r.type === 'del' ? '-' : ' ';
        return '<div class="diff-row ' + r.type + '" data-file="' + fi + '" data-row="' + ri + '"' +
          (r.type !== 'ctx' ? ' role="button" tabindex="0" aria-label="Comment on line ' + r.ln + '"' : '') + '>' +
          '<span class="ln">' + r.ln + '</span>' +
          '<span class="gutter">' + sign + '</span>' +
          '<span class="code">' + esc(r.code) + '</span>' +
          '</div>';
      }).join('');
      fileEl.innerHTML =
        '<div class="diff-file-head"><span class="fname">' + esc(f.file) + '</span></div>' +
        '<div class="diff-hunk-head">' + esc(f.hunk) + '</div>' +
  ...
  function wireDiffRows() {
    diffBoard.querySelectorAll('.diff-row.add, .diff-row.del').forEach(function (row) {
      var open = function () { openCommentBubble(row); };
  ...
  function openCommentBubble(row) {
    ...
    if (row.nextElementSibling && row.nextElementSibling.classList.contains('comment-bubble')) {
    ...
    playSound('open'); fireHaptic('light');

    var bubble = document.createElement('div');
    bubble.className = 'comment-bubble';
    var suggestBtn = rowData.suggest ? '<button type="button" class="btn-suggest">⤓ Apply suggestion</button>' : '';
    bubble.innerHTML =
    ...
    if (rowData.suggest) {
      bubble.querySelector('.btn-suggest').addEventListener('click', function () {
        playSound('success'); fireHaptic('success');
        var node = makeCommentNode('you', 'Applied suggestion ✓ — comment auto-resolved.', false, true);
  ...
  function makeCommentNode(author, body, isAgent, resolved) {
    ...
    node.className = 'comment-node' + (isAgent ? ' agent' : '') + (resolved ? ' resolved' : '');
    node.innerHTML =
      '<div class="c-meta"><span class="c-author">' + esc(author) + '</span>' +
        (resolved ? '<span class="c-badge">resolved</span>' : '') + '</div>' +
  ...
  function postComment(bubble, row, text, fi) {
    ...
    setTimeout(function () {
      ...
      rbtn.innerHTML = '<button type="button" class="btn-resolve">✓ Resolve</button>';
      agentNode.appendChild(rbtn);
      rbtn.querySelector('.btn-resolve').addEventListener('click', function () {
        playSound('resolve'); fireHaptic('success');
        agentNode.classList.add('resolved');
        var meta = agentNode.querySelector('.c-meta');
        if (!meta.querySelector('.c-badge')) {
          var badge = document.createElement('span'); badge.className = 'c-badge'; badge.textContent = 'resolved'; meta.appendChild(badge);
        }
        rbtn.remove();
      });
  ...
  verdictPop.querySelectorAll('[data-verdict]').forEach(function (b) {
    b.addEventListener('click', function () {
      ...
      playSound('send'); fireHaptic('medium');
      var model = AGENT_MODELS[Math.floor(Math.random() * AGENT_MODELS.length)];
      showToast('Review sent · ' + verdict, model, 'src/cli.ts', 'Agent picked up the review and is working through your comments…');
    });
  });
  document.addEventListener('click', function (e) {
    if (!verdictPop.hidden && !verdictPop.contains(e.target) && e.target !== sendBtn && !sendBtn.contains(e.target)) {
      verdictPop.hidden = true;
    }
  });
  ...
  toast.addEventListener('click', function () { toast.hidden = true; if (toastTimer) clearTimeout(toastTimer); });
  ...
  var SHORTCUTS = [
    ...
    { k: ['Cmd/Ctrl', '+', 'K'], a: 'Command palette (works inside text fields)' },
  ...
  function renderKeys(tbodySel) {
    ...
    SHORTCUTS.forEach(function (s) {
      ...
      var keyHtml = s.k.map(function (token) {
        return (token === '/' || token === '+') ? '<span class="dim">' + token + '</span>' : '<kbd>' + esc(token) + '</kbd>';
      }).join(' ');
      tr.innerHTML = '<td class="kbd-cell">' + keyHtml + '</td><td>' + esc(s.a) + '</td>';
      tb.appendChild(tr);
    });
  }
  ...
  document.addEventListener('focusout', function () { setTimeout(refreshVimMode, 0); });
  refreshVimMode();

  var helpOverlay = document.getElementById('help-overlay');
  function openHelp() { if (helpOverlay.hidden) { helpOverlay.hidden = false; playSound('open'); fireHaptic('medium'); document.getElementById('help-close').focus(); } }
  function closeOverlays() {
    var closed = false;
    if (!helpOverlay.hidden) { helpOverlay.hidden = true; closed = true; }
    if (!verdictPop.hidden) { verdictPop.hidden = true; closed = true; }
    document.querySelectorAll('.comment-bubble').forEach(function (b) { b.remove(); closed = true; });
    if (closed) playSound('close');
    return closed;
  }
  document.getElementById('help-close').addEventListener('click', function () { helpOverlay.hidden = true; playSound('close'); });
  helpOverlay.addEventListener('click', function (e) { if (e.target === helpOverlay) { helpOverlay.hidden = true; playSound('close'); } });

  // multi-key buffer (800ms)
  var pending = '';
  var pendingTimer = null;
  function clearPending() { pending = ''; if (pendingTimer) { clearTimeout(pendingTimer); pendingTimer = null; } }
  function setPending(key) {
    pending = key;
    if (pendingTimer) clearTimeout(pendingTimer);
    pendingTimer = setTimeout(function () { pending = ''; }, 800);
  }
  ...
  function cycleTab() {
    ...
    playSound('toggle'); fireHaptic('selection');
  }

  document.addEventListener('keydown', function (e) {
    // Cmd/Ctrl+K works even inside fields
    if ((e.metaKey || e.ctrlKey) && (e.key === 'k' || e.key === 'K')) {
      e.preventDefault(); openHelp(); clearPending(); return;
    }
    // Esc closes overlays even while typing (and blurs)
    if (e.key === 'Escape') {
      if (closeOverlays()) { clearPending(); return; }
    ...
    if (e.ctrlKey && (e.key === 'd' || e.key === 'D')) { e.preventDefault(); scrollHalf(1); return; }
    if (e.ctrlKey && (e.key === 'u' || e.key === 'U')) { e.preventDefault(); scrollHalf(-1); return; }
    if (e.ctrlKey || e.metaKey || e.altKey) return;

    // multi-key 'g' prefix
    if (pending === 'g') {
      clearPending();
      if (e.key === 'g') { window.scrollTo({ top: 0, behavior: prefersReduced ? 'auto' : 'smooth' }); return; }
      if (e.key === 't') { openThemePicker(); return; }
      if (e.key === 'v') {
        var dp = document.getElementById('diff-panel');
        if (dp) dp.scrollIntoView({ behavior: prefersReduced ? 'auto' : 'smooth', block: 'start' });
        playSound('navigate');
        return;
      }
    ...
    switch (e.key) {
      ...
      case 'j': window.scrollBy({ top: 100, behavior: prefersReduced ? 'auto' : 'smooth' }); break;
      case 'k': window.scrollBy({ top: -100, behavior: prefersReduced ? 'auto' : 'smooth' }); break;
      case 'G': window.scrollTo({ top: document.body.scrollHeight, behavior: prefersReduced ? 'auto' : 'smooth' }); break;
      case 't': cycleTab(); break;
      case 'm': toggleDiffMode(); break;
      case '/':
        e.preventDefault();
        shellInput.focus();
        document.getElementById('shell-panel').scrollIntoView({ behavior: prefersReduced ? 'auto' : 'smooth', block: 'center' });
        break;
      case 's':
        e.preventDefault();
        document.getElementById('shell-panel').scrollIntoView({ behavior: prefersReduced ? 'auto' : 'smooth', block: 'center' });
  ...
  function openThemePicker() {
    playSound('open'); fireHaptic('medium');
    var grid = document.getElementById('theme-grid');
    grid.scrollIntoView({ behavior: prefersReduced ? 'auto' : 'smooth', block: 'center' });
