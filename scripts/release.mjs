...
if (args.some((a) => a !== '--patch' && a !== '--minor' && a !== '--major' && a !== '--no-verify' && a !== '--dry-run')) {
  console.error('Usage: node scripts/release.mjs [--patch|--minor|--major] [--no-verify] [--dry-run]')
  process.exit(1)
}
...
const changelogEntry = (shortHash, subject) => `- ${shortHash}: ${subject.charAt(0).toUpperCase()}${subject.slice(1)}`
...
if (!dryRun) {
  ...
  if (behind !== '0') {
    console.error(`main is ${behind} commit(s) behind origin/main — pull first`)
    process.exit(1)
  }
  if (ahead !== '0') {
    console.error(`main is ${ahead} commit(s) ahead of origin/main — push first`)
    process.exit(1)
  }
...
try {
  ...
  tag = `v${next}`
  section = bumpType === 'major' ? 'Major' : bumpType === 'minor' ? 'Minor' : 'Patch'
} catch (error) {
...
replace('Cargo.toml', /(\[workspace\.package\][^\n]*\n(?:[^[]*\n)*?version\s*=\s*")[^"]+(")/, next)
replace('Cargo.lock', /(name = "diffing-core"\nversion = ")[^"]+(")/, next)
replace('Cargo.lock', /(name = "diffing-tui"\nversion = ")[^"]+(")/, next)
replace('site/src/layouts/BaseLayout.astro', /(__DIFFING_VERSION__ : ')[^']+(')/, next)
replace('site/src/pages/index.astro', /(__DIFFING_VERSION__ : ')[^']+(')/, next)
...
if (dryRun) {
  ...
  log(`  CHANGELOG.md: prepended ${entries.length} entr${entries.length === 1 ? 'y' : 'ies'}`)
}
...
log(`   ${process.env.GITHUB_REPOSITORY ? '' : 'https://github.com/ahmedragab20/diffing/actions'}`)
