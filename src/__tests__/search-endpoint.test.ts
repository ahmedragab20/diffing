...
const git = vi.hoisted(() => ({ getFileContent: vi.fn() }));
...
vi.mock("../lib/git.js", () => ({
  ...
  getFileContent: git.getFileContent,
  isImageFile: () => false,
  getMergeStatus: () => ({}),
  gitAddFile: () => {},
  listRepoFiles: () => [],
...
import { DEFAULTS, type DiffOptions } from "../lib/diff-options.js";
import { InMemoryCommentStore } from "../lib/comments.js";

function makeApp(diffOpts: Partial<DiffOptions> = {}) {
  return createApp(
    "/tmp/__diffing_search_test__",
    { ...DEFAULTS, ...diffOpts },
...
describe("GET /api/file-text", () => {
  beforeEach(() => {
    git.getFileContent.mockReset();
    git.getFileContent.mockReturnValue(Buffer.from("working tree"));
  });

  it("reads the working version without leaking staged/revision options", async () => {
    const res = await makeApp({ staged: true }).request(
      "/api/file-text?path=a.ts&version=working",
    );
    expect(res.status).toBe(200);
    expect((await res.json()).content).toBe("working tree");
    expect(git.getFileContent).toHaveBeenCalledWith("a.ts", "new");
  });

  it("rejects unsafe paths before reading", async () => {
    const res = await makeApp().request(
      "/api/file-text?path=../secret&version=working",
    );
    expect(res.status).toBe(400);
    expect(git.getFileContent).not.toHaveBeenCalled();
  });

  it("keeps old-version reads intact", async () => {
    await makeApp().request("/api/file-text?path=a.ts&version=old");
    expect(git.getFileContent).toHaveBeenCalledWith("a.ts", "old", {
      staged: false,
      revisions: [],
      showMode: false,
      showRevspecs: [],
...
describe("POST /api/search", () => {
  ...
  it("dispatches the files scope", async () => {
    search.searchFiles.mockResolvedValue({
      scope: "files",
      items: [{ path: "a.ts" }],
      total: 1,
      indexing: false,
    ...
    expect(search.searchFiles).toHaveBeenCalledWith("a", {
      limit: undefined,
      paths: undefined,
    });
  });
  ...
  it("forwards regex + limit to the text scope", async () => {
    search.searchContent.mockResolvedValue({
      scope: "text",
      items: [],
      total: 0,
      indexing: false,
    });
    await postSearch(makeApp(), {
      scope: "text",
      query: "foo",
      regex: true,
      limit: 10,
    });
    expect(search.searchContent).toHaveBeenCalledWith("foo", {
      limit: 10,
      regex: true,
      paths: undefined,
    });
  ...
  it("forwards queries to searchAll for all scope", async () => {
    search.searchAll.mockResolvedValue({
      scope: "all",
      items: [],
      total: 0,
      indexing: false,
    ...
    expect(search.searchAll).toHaveBeenCalledWith("bar", {
      limit: 15,
      regex: false,
      paths: undefined,
    });
  ...
  it("passes changedPaths through for the Changed-only mode", async () => {
    search.searchSymbols.mockResolvedValue({
      scope: "symbols",
      items: [],
      total: 0,
      indexing: false,
    ...
    expect(search.searchSymbols).toHaveBeenCalledWith("x", {
      limit: undefined,
      paths: ["a.ts", "b.ts"],
    });
  });
...
describe("GET /api/search/status", () => {
  ...
  it("returns the engine status", async () => {
    search.getSearchStatus.mockResolvedValue({
      available: true,
      indexing: false,
      indexedFiles: 5,
    });
