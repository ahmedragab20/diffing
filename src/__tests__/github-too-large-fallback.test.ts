...
import { isPrDiffTooLargeError, fetchPrDiffViaFilesApi, fetchPrMetadataViaGh } from '../lib/github.js'

const resolved = { owner: 'acme', repo: 'widget', pullNumber: 42, ref: '42' }

const FILES_PAYLOAD = [
  { filename: 'src/added.ts', status: 'added', additions: 2, deletions: 0, patch: '@@ -0,0 +1,2 @@\n+new\n+file', sha: 'aaa' },
  { filename: 'src/gone.ts', status: 'removed', additions: 0, deletions: 1, patch: '@@ -1 +0,0 @@\n-old', sha: 'bbb' },
  { filename: 'src/changed.ts', status: 'modified', additions: 1, deletions: 1, patch: '@@ -1 +1 @@\n-old\n+new', sha: 'ccc' },
  { filename: 'src/new.ts', previous_filename: 'src/old.ts', status: 'renamed', additions: 0, deletions: 0, patch: '@@ -1,2 +1,2 @@', sha: 'ddd' },
  { filename: 'bin/blob.png', status: 'modified', additions: 0, deletions: 0, patch: null, sha: 'eee' },
]
...
describe('isPrDiffTooLargeError', () => {
  ...
  it('returns true when stderr mentions PullRequest.diff too large', () => {
    expect(isPrDiffTooLargeError({ stderr: 'PullRequest.diff too large' })).toBe(true)
  })

  it('returns true when only the message field mentions PullRequest.diff too large', () => {
    expect(isPrDiffTooLargeError({ message: 'PullRequest.diff too large' })).toBe(true)
  })

  it('returns true when stderr matches without the PullRequest token', () => {
    expect(isPrDiffTooLargeError({ stderr: 'exceeded the maximum number of files (300)' })).toBe(true)
  })

  it('returns false for a plain HTTP 404 error', () => {
    expect(isPrDiffTooLargeError({ stderr: 'HTTP 404: Not Found', message: 'HTTP 404: Not Found' })).toBe(false)
  })

  it('returns false for a DNS failure', () => {
    expect(isPrDiffTooLargeError({ message: 'getaddrinfo ENOTFOUND github.com' })).toBe(false)
  })
...
describe('fetchPrDiffViaFilesApi', () => {
  ...
  it('assembles the unified diff from the files API payload', async () => {
    const diff = await fetchPrDiffViaFilesApi(resolved)
  ...
  it('requests 100 files per page from the files API', async () => {
    await fetchPrDiffViaFilesApi(resolved)
    const calls = await readCallLog()
    expect(calls.some((argv) => argv.join(' ').match(/pulls\/42\/files\?per_page=100/))).toBe(true)
  })
})
...
describe('fetchPrMetadataViaGh fallback (integration)', () => {
  ...
  it('attempts the fast path before falling back to the files API', async () => {
    ...
    expect(calls.some((argv) => argv.join(' ').includes('pulls/42/files'))).toBe(true)
  })
})
