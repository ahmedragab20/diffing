...
export function AiModelPicker({ onOpenAssistant, onManage }: { onOpenAssistant?: () => void; onManage?: () => void }) {
	...
	const visible = useMemo(() => {
		...
		return models.filter((model) => `${model.displayName} ${aiSourceLabel(model.sourceId)} ${model.sourceId} ${model.credentialRoute} ${model.providerId}`.toLowerCase().includes(needle));
	}, [models, query]);
	if (!ai) return null;
				...
				<div className="ai-model-menu">
					...
					<div className="ai-model-options">
						{visible.map((model, index) => {
							...
							const previousGroup = previous ? `${aiSourceLabel(previous.sourceId)} · ${previous.credentialRoute}` : "";
					...
					<div className="ai-model-session-note"><span>Selected model</span><small>Your choice is remembered across reloads.</small></div>
