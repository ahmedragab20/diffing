...
function AiAssistantRailOpen({ onClose, surface, context, title = "Ask AI", ai }: AiAssistantRailProps & { ai: NonNullable<ReturnType<typeof useOptionalAi>> }) {
	...
	return (
		<aside className="ai-assistant-rail" aria-label={title} style={{ width: localWidth }}>
			...
			<div className="ai-context-bar"><div className="ai-context-chips"><span>{context.kind}</span>{"filePath" in context && context.filePath && <span title={context.filePath}>{context.filePath}</span>}{"version" in context && <span>v{context.version}</span>}{attachmentPaths.map((path) => <button type="button" className="ai-attachment-chip" key={path} title={`Remove ${path}`} aria-label={`Remove ${path}`} onClick={() => setPrompt((value) => value.replace(`@${path} `, "").replace(`@${path}`, ""))}><Paperclip size={9} />{path}<X size={9} /></button>)}</div><details className="ai-share-details"><summary>Context being shared</summary><p>Only this review context is sent. No unrelated files, mockups, credentials, or hidden state.</p></details></div>
