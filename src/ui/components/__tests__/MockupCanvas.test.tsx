...
import type { ViewportPx } from "../MockupAnchors";
...
const baseProps = {
  ...
  viewport: 1280 as ViewportPx,
  comments: [] as MockupComment[],
  viewOnly: false,
  zen: false,
  staleIds: undefined as Set<string> | undefined,
  onIframeLoad: vi.fn(),
...
describe("MockupCanvas", () => {
  ...
  it("view-only widens the sandbox for interactivity and suppresses the hover outline", () => {
    renderCanvas({
      viewOnly: true,
      hover: {
        kind: "block",
    ...
    expect(document.querySelector(".mockup-outline")).toBeNull();
  });

  it("view-only hides pins, threads, and the composer (comments disabled)", () => {
    renderCanvas({
      viewOnly: true,
  ...
  it("hides pins whose anchor element is missing (stale) from the canvas", () => {
    renderCanvas({
      comments: [
        comment({ id: "c1" }),
        comment({ id: "c2", selector: "button.gone" }),
      ],
      staleIds: new Set(["c2"]),
    });
    const pins = screen.getAllByRole("button", { name: /^Comment \d:/ });
    expect(pins).toHaveLength(1);
    expect(pins[0]).toHaveAccessibleName(expect.stringContaining("button.pay"));
  });

  it("zen frames the mockup at full width", () => {
    renderCanvas({ zen: true });
    const frame = document.querySelector(".mockup-frame") as HTMLElement;
