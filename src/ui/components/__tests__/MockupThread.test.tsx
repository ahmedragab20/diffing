...
import { MockupThread, type MockupThreadProps } from "../MockupThread";
...
vi.mock("../CommentForm", () => ({
  CommentForm: ({
    ...
    return (
      <div
        data-testid="mockup-comment-form"
        data-ai-surface={aiSurface ?? ""}
        data-ai-kind={aiContext?.kind ?? ""}
      >
        <textarea
...
function renderThread(
  c: MockupComment,
  overrides: Partial<MockupThreadProps> = {},
) {
  return render(
    <MockupThread
      index={1}
...
describe("MockupThread", () => {
  ...
  it("shows Apply Suggestion for a suggestion fence and reports apply errors", async () => {
    const onApplySuggestion = vi
      .fn()
  ...
  it("passes mockup AI context into the reply composer without running inference", () => {
    renderThread(comment(), {
      aiContext: {
        kind: "mockup",
        mockupId: "m1",
    ...
    fireEvent.click(screen.getByRole("button", { name: "Reply..." }));
    const form = screen.getByTestId("mockup-comment-form");
    expect(form).toHaveAttribute("data-ai-surface", "mockup");
    expect(form).toHaveAttribute("data-ai-kind", "mockup-thread");
  });
