...
export function classifyNavigation(
  ...
  const canScrollMainDiff =
    inDiffFile &&
    !ctx.staged &&
    !ctx.customMode &&
    ctx.changedKeys.has(`${input.path}:${input.line}`);
 ...
 */
export function decodeByteRanges(
  content: string,
  byteRanges: MatchRangeLike[],
): [number, number][] | null {
  ...
  for (const r of byteRanges) {
    const [bs, be] = r;
    if (bs == null || be == null || be <= bs || bs < 0 || be > byteLen)
      continue;
    if (ascii) {
      out.push([bs, be]);
    } else {
...
export function highlightRanges(
  ...
  const clamped = ranges
    .map(
      ([s, e]) =>
        [Math.max(0, s), Math.min(content.length, e)] as [number, number],
    )
    .filter(([s, e]) => e > s)
    .sort((a, b) => a[0] - b[0]);
