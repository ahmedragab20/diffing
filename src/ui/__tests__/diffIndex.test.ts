...
describe("classifyNavigation", () => {
  const ctx = (
    over: Partial<Parameters<typeof classifyNavigation>[1]> = {},
  ) => ({
  ...
  it("file in diff -> scrollFile", () => {
    expect(classifyNavigation({ kind: "file", path: "a.ts" }, ctx())).toEqual({
      type: "scrollFile",
      path: "a.ts",
    });
  });
  ...
  it("file not in diff -> preview", () => {
    expect(classifyNavigation({ kind: "file", path: "z.ts" }, ctx())).toEqual({
      type: "preview",
      path: "z.ts",
    });
  });
  ...
  it("changed line in a plain working-tree diff -> scrollLine", () => {
    expect(
      classifyNavigation(
        { kind: "line", path: "a.ts", line: 10, match: "foo" },
        ctx(),
      ),
    ).toEqual({
      type: "scrollLine",
  ...
  it("line outside changed keys in a diff file -> preview", () => {
    expect(
      classifyNavigation({ kind: "line", path: "a.ts", line: 99 }, ctx()),
    ).toEqual({
      type: "preview",
      path: "a.ts",
      line: 99,
  ...
  it("line outside changed keys in staged mode -> preview", () => {
    expect(
      classifyNavigation(
        { kind: "line", path: "a.ts", line: 99 },
        ctx({ staged: true }),
      ),
    ).toEqual({
      type: "preview",
  ...
  it("line outside changed keys in custom revision mode -> preview", () => {
    expect(
      classifyNavigation(
        { kind: "line", path: "a.ts", line: 99 },
        ctx({ customMode: true }),
  ...
  it("changed line in staged mode -> preview", () => {
    expect(
      classifyNavigation(
        { kind: "line", path: "a.ts", line: 10 },
        ctx({ staged: true }),
      ),
    ).toEqual({
      type: "preview",
  ...
  it("changed line in custom revision mode -> preview", () => {
    expect(
      classifyNavigation(
        { kind: "line", path: "a.ts", line: 10 },
        ctx({ customMode: true }),
