...
import {
  buildExpandedFileSearchEntries,
  buildFileSearchCorpus,
  buildFileSearchEntries,
} from "../useDiffSearch";
...
function fileA(): FileDiffMetadata {
  ...
  const deletionLines = Array.from(
    { length: 20 },
    (_, i) => `placeholder-d-${i}`,
  );
...
describe("buildExpandedFileSearchEntries", () => {
  it("builds the full old/new corpus with new line numbers, deletions, and one unchanged copy", () => {
    const filler = Array.from({ length: 10 }, (_, i) => `filler ${i + 1}`);
    const oldLines = ["removed near", ...filler, "needle far"];
    const newLines = ["added near", ...filler, "needle far"];
    const entries = buildExpandedFileSearchEntries(
      "src/a.ts",
      oldLines.join("\n"),
      newLines.join("\n"),
    );
    ...
    expect(normalized).toEqual(
      expect.arrayContaining([
        ...
        },
        {
          ...
          lineNumber: 1,
          side: "deletions",
          content: "removed near",
        },
      ]),
    );
    for (const line of filler) {
      expect(normalized.filter((entry) => entry.content === line)).toHaveLength(
        1,
      );
    }
  });

  it("keeps identical versions searchable", () => {
    expect(
      buildExpandedFileSearchEntries(
        "src/a.ts",
        "same\nneedle",
...
describe("buildFileSearchEntries", () => {
  ...
  it("emits context lines once on the additions side with new-file numbering", () => {
    const contextEntries = buildFileSearchEntries(fileA()).filter((e) =>
      e.content.startsWith("ctx-"),
    );
  ...
  it("skips blank and whitespace-only lines", () => {
    ...
    expect(entries.some((e) => e.content === "   " || e.content === "")).toBe(
      false,
    );
...
describe("buildFileSearchCorpus", () => {
  it("concatenates file entries in order with each filePath equal to the file name", () => {
    ...
    expect(
      corpus.every(
        (e) =>
          e.filePath === (e.filePath === "src/a.ts" ? "src/a.ts" : "src/b.ts"),
      ),
    ).toBe(true);
  });
});
