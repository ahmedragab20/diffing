...
export function useSearch({
  ...
  const enabled = open && query.trim().length >= minLength(scope);
  ...
  const result = useQuery<SearchResponse>({
    queryKey: [
      ...
      scope,
      trimmed,
      scope === "text" ? regex : false,
      changedOnly,
      changedOnly ? changedPaths : null,
    ],
  ...
  return {
    ...result,
    // Never render or activate hits for a different input or a refreshing snapshot.
    data:
      enabled && !pending && !result.isFetching && !result.isError
        ? result.data
        : undefined,
    error: enabled && !pending ? result.error : null,
    enabled,
    pending,
    revision,
