...
function makeBox(quote: Quote, pal: Palette): Box {
  ...
  const innerWidth =
    Math.max(...textLines.map((l) => l.length), authorStr.length, 28) + 4;
...
const animTypewriter: Anim = async (b) => {
  ...
  for (const line of b.textLines) {
    ...
    stdout.write(
      " ".repeat(b.innerWidth - 4 - line.length) + `  ${b.pal.base}│${R}\n`
    );
  }
...
const animWaveReveal: Anim = async (b) => {
  ...
  for (const line of b.textLines) {
    ...
    stdout.write(
      contentRow(
        b,
        line.slice(0, half).padEnd(line.length),
        b.pal.base,
        b.pal.faint
      ) + "\n"
    );
...
const animPulseBorder: Anim = async (b) => {
  ...
  for (let s = 0; s < stages.length; s++) {
    ...
    for (const line of stages[s])
      stdout.write((s > 0 ? CLR : "") + line + "\n");
    ...
    for (const line of b.textLines) {
      if (rate === 0) {
        ...
        stdout.write(
          clr +
            pad +
            `${b.pal.base}│${R}  ${b.pal.text}${inner}${R}${fill}  ${b.pal.base}│${R}\n`
        );
      }
    }
...
const animMatrixRain: Anim = async (b) => {
  ...
  const RAIN_HEIGHT = 6;
  const streams: Array<Array<{ char: string } | string>> = Array.from(
    { length: RAIN_HEIGHT },
    () => Array(width).fill(" ")
  );
