...
export type EditorChoice = "vscode" | "zed" | "vim" | "neovim" | "ghostty";
...
export function resolveEditorCommand(
	...
	switch (editor) {
		...
		case "ghostty":
			// Launch `nvim` inside a new Ghostty terminal window.
			return ghosttyEditorCommand(absolutePath, platform);
		default:
			return null;
	}
}
...
function ghosttyEditorCommand(
	absolutePath: string,
	platform: NodeJS.Platform,
): EditorCommand | null {
	if (platform === "darwin") {
		// Ghostty's CLI window actions (+new-window / +open) are not supported on
		// macOS, so the official route is `open -na Ghostty --args -e <cmd>`,
		// which opens a new window running the command. `-n` forces a fresh
		// launch so the args are honoured even when an instance is already
		// running (plain `open -a` would just activate the existing one).
		return {
			cmd: "open",
			args: ["-na", "Ghostty", "--args", "-e", "nvim", absolutePath],
		};
	}

	if (platform === "linux") {
		return { cmd: "ghostty", args: ["-e", "nvim", absolutePath] };
	}

	// Ghostty has no Windows build — fall back to the caller's default opener.
	return null;
}
...
function terminalEditorCommand(
	...
	if (platform === "darwin") {
		const appleScriptPath = absolutePath
			.replace(/\\/g, "\\\\")
			.replace(/"/g, '\\"');
		return {
			cmd: "osascript",
