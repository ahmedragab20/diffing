...
async function runSearch(body: SearchBody): Promise<unknown> {
  ...
  if (scope === 'text') return searchContent(query, { limit, regex: !!body.regex, paths })
...
export function loadTuiPreview(body: PreviewBody): unknown {
  if (typeof body.path !== 'string' || !body.path) {
    throw new Error('preview path is required')
  }
  const buffer = getFileContent(body.path, 'new')
  if (!buffer) return { path: body.path, content: '', missing: true, binary: false, truncated: false }

  const sample = buffer.subarray(0, Math.min(buffer.length, 8192))
  if (sample.includes(0)) {
    return { path: body.path, content: '', missing: false, binary: true, truncated: false }
  }
...
export async function startTuiSearchBridge(): Promise<TuiSearchBridge> {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'POST' && request.url === '/preview') {
        ...
        sendJson(response, 200, loadTuiPreview(body))
        return
      }
      if (request.method === 'POST' && request.url === '/track') {
        ...
        if (typeof body.path === 'string') {
          ...
          await trackSelection(typeof body.query === 'string' ? body.query : '', body.path)
        }
  ...
  return {
    ...
    capability,
    close: () => new Promise<void>((resolveP) => server.close(() => resolveP())),
  }
}
