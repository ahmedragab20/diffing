...
import type { AiConversationContextLabel, AiConversationTurn, AiSurface } from "./types.js";
...
export type { AiConversationContextLabel, AiConversationTurn } from "./types.js";
...
export interface AiConversationStore {
	list(scope?: { surface?: AiSurface; scopeKey?: string }): Promise<AiConversationSummary[]>;
	get(id: string): Promise<AiConversation | null>;
	create(input: AiConversationCreateInput): Promise<AiConversation>;
	update(id: string, input: AiConversationUpdateInput): Promise<AiConversation | null>;
	remove(id: string): Promise<boolean>;
}

const SURFACES = new Set<AiSurface>(["diff", "pr-diff", "plan"]);

function boundedText(value: unknown, maxBytes: number): string | undefined {
	if (typeof value !== "string") return undefined;
	if (Buffer.byteLength(value, "utf8") <= maxBytes) return value;
	let end = value.length;
	while (end > 0 && Buffer.byteLength(value.slice(0, end), "utf8") > maxBytes) end -= 256;
	return value.slice(0, Math.max(0, end));
}

function normalizeContext(value: unknown): AiConversationContextLabel | undefined {
	if (!value || typeof value !== "object") return undefined;
	const candidate = value as Record<string, unknown>;
	const context: AiConversationContextLabel = {};
	if (typeof candidate.kind === "string") context.kind = candidate.kind.slice(0, 40);
	if (typeof candidate.filePath === "string") context.filePath = candidate.filePath.slice(0, 512);
	if (typeof candidate.label === "string") context.label = candidate.label.slice(0, 160);
	if (typeof candidate.version === "number" && Number.isFinite(candidate.version)) context.version = Math.max(0, Math.floor(candidate.version));
...
function normalizeTurn(value: unknown): AiConversationTurn | null {
	...
	if (!text) return null;
	return {
		id: typeof candidate.id === "string" && candidate.id ? candidate.id.slice(0, 80) : randomUUID(),
		role: candidate.role,
		text,
...
function normalizeConversation(value: unknown): AiConversation | null {
	if (!value || typeof value !== "object") return null;
	const candidate = value as Record<string, unknown>;
	if (typeof candidate.id !== "string" || !candidate.id || !SURFACES.has(candidate.surface as AiSurface)) return null;
	const now = Date.now();
	...
	const turns = Array.isArray(candidate.turns)
		? candidate.turns.map(normalizeTurn).filter((turn): turn is AiConversationTurn => !!turn).slice(-AI_CONVERSATION_MAX_TURNS)
		: [];
	return {
...
function prune(conversations: AiConversation[], now = Date.now()): AiConversation[] {
	return conversations
		.filter((conversation) => now - conversation.updatedAt <= AI_CONVERSATION_RETENTION_MS)
...
function boundedStore(conversations: AiConversation[]): AiConversation[] {
	...
	while (next.length && Buffer.byteLength(JSON.stringify(next), "utf8") > AI_CONVERSATIONS_MAX_STORE_BYTES) next = next.slice(0, -1);
	return next;
}
...
export class InMemoryAiConversationStore implements AiConversationStore {
	...
	async list(scope?: { surface?: AiSurface; scopeKey?: string }): Promise<AiConversationSummary[]> {
		this.conversations = boundedStore(this.conversations);
		return this.conversations
			.filter((conversation) => !scope?.surface || conversation.surface === scope.surface)
			.filter((conversation) => !scope?.scopeKey || conversation.scopeKey === scope.scopeKey)
			.map(summary);
	}
	...
	async update(id: string, input: AiConversationUpdateInput): Promise<AiConversation | null> {
		const current = await this.get(id);
		if (!current) return null;
		const next: AiConversation = {
			...current,
			title: input.title === undefined ? current.title : (boundedText(input.title, 160) || "New conversation"),
			draft: input.draft === undefined ? current.draft : boundedText(input.draft, 16 * 1024),
			modelId: input.modelId === undefined ? current.modelId : boundedText(input.modelId, 512),
			turns: input.turns === undefined ? current.turns : input.turns.map(normalizeTurn).filter((turn): turn is AiConversationTurn => !!turn).slice(-AI_CONVERSATION_MAX_TURNS),
			updatedAt: Date.now(),
		};
		this.conversations = boundedStore(this.conversations.map((conversation) => conversation.id === id ? next : conversation));
		return next;
	}
...
export class FileAiConversationStore implements AiConversationStore {
	...
	private async load(): Promise<AiConversation[]> {
		...
		try {
			...
			this.conversations = Array.isArray(parsed) ? parsed.map(normalizeConversation).filter((conversation): conversation is AiConversation => !!conversation) : [];
		} catch {
			this.conversations = [];
		}
		return this.conversations;
	...
	private async save(next: AiConversation[]): Promise<void> {
		...
		this.mutationQueue = this.mutationQueue.catch(() => {}).then(async () => {
	...
	async update(id: string, input: AiConversationUpdateInput): Promise<AiConversation | null> {
		...
		await this.save(conversations.map((conversation) => conversation.id === id ? updated : conversation));
		return updated;
	}
	...
	async remove(id: string): Promise<boolean> {
		...
		if (removed) await this.save(conversations.filter((conversation) => conversation.id !== id));
		return removed;
	}
}
