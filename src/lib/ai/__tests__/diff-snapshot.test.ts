...
function capture(text: string, omissions: string[] = []) {
  return {
...
function code(fn: () => unknown, expected: AiSnapshotError["code"]) {
  try {
    fn();
    throw new Error("expected error");
  } catch (e) {
    expect(e).toBeInstanceOf(AiSnapshotError);
    expect((e as AiSnapshotError).code).toBe(expected);
  }
}
const context = (overrides: Partial<AiDiffContext> = {}): AiDiffContext => ({
  kind: "diff",
  patch: "forged browser text",
  ...overrides,
});
...
describe("resolveDiffSnapshot", () => {
  it("uses captured patch, keeps focused diff whole, and preserves duplicate occurrences", () => {
    ...
    expect(
      resolved.snapshot.manifest.sources.every(
        (s) => s.complete === true && s.representation === "unified-patch",
      ),
    ).toBe(true);
  ...
  it("matches either side of renames and supports Git quoted paths", () => {
    ...
    expect(
      resolveDiffSnapshot(
        context({ kind: "file", filePath: "old name.ts" }),
        capture(renamed),
        "/repo",
    ...
    expect(
      resolveDiffSnapshot(
        context({ kind: "file", filePath: "new name.ts" }),
        capture(renamed),
        "/repo",
      ).snapshot.manifest.sources,
    ).toHaveLength(1);
    const quoted =
      'diff --git "a/weird\\303\\251 name.ts" "b/weird\\303\\251 name.ts"\n--- "a/weird\\303\\251 name.ts"\n+++ "b/weird\\303\\251 name.ts"\n';
    expect(
      resolveDiffSnapshot(
        context({ kind: "file", filePath: "weirdé name.ts" }),
        capture(quoted),
        "/repo",
      ).snapshot.manifest.sources,
    ).toHaveLength(1);
  ...
  it("carries captured originals alongside the patch and says offsets are not file lines", () => {
    ...
    const resolved = resolveDiffSnapshot(
      context(),
      { ...capture(text), originals },
      "/repo",
    );
    const sources = resolved.snapshot.manifest.sources;
    expect(sources.filter((s) => s.representation === "original")).toHaveLength(
      2,
    );
    expect(
      sources.filter((s) => s.representation === "unified-patch"),
    ).toHaveLength(1);
    // With originals present the capture must stop claiming no coverage.
    expect(resolved.snapshot.manifest.omissions.join(" ")).toContain(
      "cite original-file lines",
    );
    expect(resolved.snapshot.manifest.omissions.join(" ")).not.toContain(
      "Original-file coverage is not established",
    );
  ...
  it("keeps only the scoped path's originals and refuses an unsafe original path", () => {
    ...
    const scoped = resolveDiffSnapshot(
      context({ kind: "file", filePath: "src/a.ts" }),
      {
        ...capture(text),
        originals: [original("src/a.ts"), original("src/b.ts")],
      },
      "/repo",
    );
    expect(
      scoped.snapshot.manifest.sources
        .filter((s) => s.representation === "original")
        .map((s) => s.path),
    ).toEqual(["src/a.ts"]);
    code(
      () =>
        resolveDiffSnapshot(
          context(),
          { ...capture(text), originals: [original("../escape.ts")] },
          "/repo",
        ),
      "invalid",
    );
  ...
  it("rejects unsafe paths, missing scope, stale hashes, malformed/combined patches, and oversized patches", () => {
    ...
    code(
      () =>
        resolveDiffSnapshot(
          context({ kind: "file", filePath: "missing.ts" }),
          capture(patch("x")),
          "/repo",
        ),
      "missing",
    ...
    code(
      () =>
        resolveDiffSnapshot(context(), {
          ...capture(patch("x")),
          identity: { ...identity, patchHash: "bad" },
        }),
      "stale",
    );
    code(
      () => resolveDiffSnapshot(context(), capture("diff --combined a.ts\n")),
      "unsupported",
    );
    code(
      () => resolveDiffSnapshot(context(), capture("not a patch\n")),
      "unsupported",
    );
    ...
    code(
      () =>
        resolveDiffSnapshot(context(), capture(patch("../outside")), "/repo"),
      "invalid",
    );
  });
});
